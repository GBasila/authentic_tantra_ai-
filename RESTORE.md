...
> Дата создания: 2026-05-30 · Последняя редакция: 2026-07-17
...
# 8. ЧТО УЖЕ ПОСТРОЕНО (по состоянию на 2026-07-17)

## Обновления Июль 2026

Крупные добавления с 2026-05-30:

### 📚 Библиотека знаний бренда (`/library/`)
- Инфраструктура интеллектуального фундамента бренда — не для внешнего использования
- `README.md` — устав библиотеки, 8-разделная структура анализа
- `knowledge_base.md` — центральный кросс-референсный индекс идей (v0.2)
- `books/` — по одному файлу на источник:
  - `01_marciniak_bringers_of_the_dawn.md` — 8 разделов, 35 тегов
  - `02_dillon_tantra_path_to_bliss.md` — 8 разделов, 40+ тегов + принципиальные расхождения с методологией
  - `02_dillon_praktiki.md` + Word — компиляция всех 25 практик из книги с оценкой «берём / переосмысливаем / не берём»
- Jewel in the Lotus — в обработке (медленный OCR)

### 🧘 Библиотека медитаций (`/content/meditations/`)
- **Мини-курс «Дом внутри · Пять практик»** — готовый 5-дневный продукт
- 5 брендовых guided meditations (~20 минут каждая), одна архитектура из 8 частей:
  1. **Возвращение к себе** (v2 master edition) — любовь как факт присутствия
  2. **Голос** — слышать себя под слоями чужих голосов
  3. **Простор** — растождествление, первая природа
  4. **Из источника** — формирование желания из глубины
  5. **Встреча** — быть с другим, не теряя себя
- Каждая — в .md (исходник) и .docx (для команды / записи)
- `README.md` — концепция мини-курса, инструкции по записи аудио, варианты упаковки

### 🏛 Архитектура флагманской программы (`/strategy/flagship_program_architecture.md` + .docx)
- Фундаментальный документ на 5 столпов главного продукта
- Отвергает «Отношения / Деньги / Здоровье» как ложную рамку
- 5 столпов: Тело как дом · Память целостности · Слышать себя · Присутствие · Встреча без исчезновения
- Карта человека в 5 уровнях глубины
- Карта трансформации участницы (4 фазы)
- 10 принципов построения 8-недельной программы

### 📢 Шаблон анонсов воркшопов (`/templates/tantra_workshop_announcement_template.md`)
- 4-блочный формат: шапка / тема / что делаем / что вас ждёт + регистрация
- Правила форматирования для Telegram (жирные заголовки, плотные буллеты)
- Правила переносов (только между смысловыми абзацами)
- Блок 5: скрипт видео-кружочка на 1 минуту
- Эталонные примеры EN + RU на тему «Weaving Polarities»

### 🎨 Постеры + логотип Арканы
- `v6-en-weekly-arcana-right.png` — постер с логотипом Арканы в правом нижнем углу
- QR перенесён выше, чтобы освободить место под логотип-партнёра
- `arcana-logo.png` — рабочий актив в `/website/assets/brand/poster-drafts/`

### 🎬 Видео с брендовыми субтитрами
- `/content/instagram/ostrov_video_subtitled.mp4` — Reels с текстом «Не скучно ли жить на острове» в брендовом голосе
- InstrumentSerif Italic, кремовый цвет, взвешенный тайминг по количеству слов на абзац

### 📖 Другие стратегические документы
- `/strategy/methodology_authentic_tantra.md` — методология
- `/strategy/pema_gitama_source_of_truth.md` — Каула-фундамент
- `/strategy/visual_dna_authentic_tantra.md` — визуальная ДНК
- `/strategy/bondarenko_playbook_for_authentic_tantra.md` — техники хука без манипуляции
- `/strategy/brand_naming_research.md` — исследование альтернативных имён
- `/strategy/know_thyself_konspekt.md` + Word — конспект 15 видео Know Thyself Thinktank
- `/strategy/Margo_Anand_Practice_Base.docx` + сопутствующее — 51 практика тантры

### 🎓 Библиотека уроков v1
- `/content/lesson_library_v1.md` — 15 полных уроков по 7-разделной структуре (~15,000 слов)
- `/content/scripts_ready_to_record_v1.md` + Word — 15 готовых к записи скриптов

## 🔥 Важное брендовое решение · Июль 2026

**Термин «Возвращение» постепенно выводится из активного бренд-словаря** — потому что несёт смысл «шага назад». Вместо этого используем: «приход», «дом внутри», «узнавание», «прибытие».

Что нужно постепенно переименовать (не сделано):
- Медитация «Возвращение к себе» (может стать «Дом» или «Приход к себе»)
- Раздел VIII «Возвращение» в скриптах медитаций (может стать «Выход» / «Приземление»)
- Возможно в архитектуре флагманской программы

Оставлено пока для решения Виктории.

---

## Исходное состояние (2026-05-30)
